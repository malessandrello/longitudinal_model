...
There is no balance of sex within treatment. Most of the subjects (64%) of treatment 0 correspond to sex 1 and most of the subjects (66%) of treatment 1 correspond to sex 0. If there is an effect of treatment in the DLPFC activity, it could be biased by any sex effect.
...
The data will be modeled using a linear mixed effect model using the lmer function of the lmerTest package:
...
In this case, the time contributes to the DLPFC with a 0.12 yearly increase, as in the previous model, and for those induviduals that belong to the group sex = 1, a constant factor of 0.23 is added. Now, let's consider adding the treatment effect:
...
```
***
It can be seen that the model that includes the sex effect has a better performance than the one that does not include it.
It is also noteworthy that both models present a shoulder on the density curve of the posterior predictive check.
Interestingly, if we exclude 51 people that have a DLPFC activation higher than 1.5 at the beginning of the study, the shoulder tend to flatten. This is expected because the initial value predicted by the model is 0.44 for sex = 0 and 0.44 + 0.23 for sex = 1.
...
```
***
As it can be seen, this group of individuals with high initial DLPFC activation decrease it over time in average. Therefore, a model that predicts a yearly increase will not be useful for this particular group.  

## Conclusion

The resultant model for the fixed effects on DLPFC measures of this longitudinal study is the following:  
For sex = 1:
$$
